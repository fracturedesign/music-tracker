...
const THEMES = {
  calm: {
    ...
    indigo: "#818cf8", deep: "#6366f1",
    green: "#34d399", greenS: "#6ee7b7", flame: "#fb923c",
    accentGrad: "linear-gradient(135deg,#818cf8,#6366f1)",
    accentGlow: "rgba(99,102,241,0.6)",
    accentAlpha: "rgba(129,140,248,0.16)",
    accentAlpha2: "rgba(129,140,248,0.12)",
...
const MOOD_EMOJI = ["","😓","😐","🙂","😊","🔥"];
const PROMPTS = [
  "Make something beautiful today.","Chase the sound in your head.",
  "Press record, see what happens.","One loop can become a song.",
  "Start before you feel ready.","Trust your ears today.",
  "Finish something today.","Show up for the music.","Turn an idea into a take.",
];
...
function heatColor(mins, heat) {
  if(!mins) return heat[0];
  if(mins<45) return heat[1];
  if(mins<90) return heat[2];
  if(mins<150) return heat[3];
  return heat[4];
}
function fmtDur(min) {
...
const storage = {
  ...
  async set(key,value) {
    await fetch(`/api/data/${key}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({value})});
  },
};
    ...
    <div className="overlay" style={{alignItems:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&close()}>
      <div style={{background:C.surf,borderRadius:22,border:`1px solid ${C.lineS}`,width:"100%",maxWidth:440,display:"flex",flexDirection:"column",animation:"mtpop .22s ease"}}>
          ...
          {[["preview","Preview"],["edit","Edit"]].map(([m,label])=>(
            <button key={m} onClick={m==="preview"?toPreview:toEdit}
              style={{fontSize:12.5,fontWeight:600,padding:"7px 16px",borderRadius:10,border:"none",cursor:"pointer",
                background:mode===m?C.accentAlpha:"transparent",color:mode===m?C.indigo:C.faint}}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",padding:"14px 20px",borderTop:`1px solid ${C.line}`}}>
          <button onClick={close} style={{background:C.accentGrad,border:"none",borderRadius:12,color:"#fff",padding:"11px 22px",fontSize:14,fontWeight:600,cursor:"pointer"}}>
            Save &amp; close
          </button>
        </div>
      </div>
            ...
            {cells.map((ds,i)=>{
              ...
              return (
                <button key={ds} type="button" disabled={future} onClick={()=>pick(ds)} style={{
                  ...
                  background:isSel?C.accentGrad:"transparent",
        ...
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:4}}>
            ...
            <button onClick={()=>onSubmit(form)} style={{
              ...
              fontSize:16,fontWeight:700,color:"#fff",background:C.accentGrad,
              boxShadow:`0 8px 22px -8px ${C.accentGlow}`,
          ...
          {chips.map(c=>{
            ...
            return (
              <button key={c} onClick={()=>setFilter(c)} style={{
                ...
                border:active?`1px solid ${C.accentBorder}`:`1px solid ${C.lineS}`,
                background:active?C.accentAlpha:"transparent",color:active?C.indigo:C.muted,
              }}>{label}<span style={{opacity:.55,fontWeight:500}}>{countFor(c)}</span></button>
            );
          })}
        </div>
                ...
                <div key={s.id} style={{background:C.surf2,border:`1px solid ${C.line}`,borderRadius:14,padding:"13px 14px",display:"flex",alignItems:"center",gap:10}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      {s.project&&<span style={{fontSize:12.5,fontWeight:600,color:C.indigo,background:C.accentAlpha2,borderRadius:7,padding:"3px 9px",maxWidth:220,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.project}{proj?.notes?" 📝":""}</span>}
...
/* ─── settings sheet ─── */
function SettingsSheet({currentTheme,onThemeChange,onClose}) {
  const C=useTheme(); const {iconBtn}=getStyles(C);
  return (
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
          ...
          {Object.entries(THEMES).map(([key,T])=>{
            ...
            return (
              <button key={key} onClick={()=>onThemeChange(key)} style={{
                ...
                border:active?`1px solid ${C.accentBorder}`:`1px solid ${C.line}`,
                background:active?C.accentAlpha:C.surf2,
              }}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    {T.heat.map((c,i)=><div key={i} style={{flex:1,height:6,borderRadius:3,background:c}}/>)}
                  </div>
                </div>
        ...
        <div style={{fontSize:11.5,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:C.faint,marginBottom:10}}>About</div>
        <div style={{fontSize:13,color:C.muted,lineHeight:1.6}}>Studio Log · v2<br/>Theme is saved per device.</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try{const p=await storage.get("music_projects");if(p?.value){const raw=JSON.parse(p.value);setProjects(raw.map(x=>typeof x==="string"?{name:x,notes:""}:x));}}catch{}
  ...
  const flash=msg=>{setToast(msg);clearTimeout(toastTimer.current);toastTimer.current=setTimeout(()=>setToast(""),1900);};
  ...
  useEffect(()=>{
    const iv=setInterval(async()=>{
      try{
        ...
        setTimer(local=>{
          if(remote.phase!==local.phase||remote.endsAt!==local.endsAt||remote.remaining!==local.remaining)return remote;
          return local;
        });
      }catch{}
    },3000);
  ...
  const heatmapCells=(()=>{
    ...
    const cells=[];
    for(let i=0;i<84;i++){const d=new Date(startD);d.setDate(startD.getDate()+i);const ds=toDateStr(d);cells.push({ds,mins:sessionsByDate[ds]||0,future:d>todayD});}
    return cells;
  })();
  ...
  const weekStrip=(()=>{
    ...
    return Array.from({length:7}).map((_,i)=>{
      const d=new Date(mon);d.setDate(mon.getDate()+i);const ds=toDateStr(d);
  ...
  const addProject=async()=>{const name=newProject.trim();if(!name||projects.find(p=>p.name===name))return;const next=[...projects,{name,notes:""}];setProjects(next);await persistProjects(next);setNewProject("");};
  const removeProject=async name=>{const next=projects.filter(p=>p.name!==name);setProjects(next);await persistProjects(next);setConfirmDelProject(null);};
  ...
  return (
    ...
    <div className="app" style={{background:C.bg}}>
      ...
      {settingsOpen&&<SettingsSheet currentTheme={themeKey} onThemeChange={changeTheme} onClose={()=>setSettingsOpen(false)}/>}
      ...
      <div className="card" style={{...card,padding:22,
        ...
        {timer.phase==="setup" ? (
          <>
            ...
            <div style={{display:"flex",gap:10}}>
              ...
              <button onClick={()=>{const v=Number(customMin);if(v>0)startCountdown(v);}} disabled={!(Number(customMin)>0)} style={{
                ...
                background:C.accentGrad,opacity:Number(customMin)>0?1:0.4,fontFamily:"var(--font-sans)",
              }}>Start</button>
            </div>
          </>
        ) : showTimerUI ? (
          <>
            ...
            {timer.phase==="done" ? (
              <button onClick={logCountdown} style={{width:"100%",padding:15,borderRadius:14,border:"none",cursor:"pointer",fontFamily:"var(--font-sans)",fontSize:15.5,fontWeight:700,color:"#0b0b10",background:"linear-gradient(135deg,#6ee7b7,#34d399)"}}>
                ✓ Log {timerTargetMin}m session
              </button>
            ) : (
              <div style={{display:"flex",gap:10}}>
                <button onClick={timer.phase==="running"?pauseCountdown:resumeCountdown} style={{flex:1,padding:15,borderRadius:14,cursor:"pointer",border:`1px solid ${C.lineS}`,background:"transparent",color:C.muted,fontFamily:"var(--font-sans)",fontSize:15.5,fontWeight:600}}>
                  {timer.phase==="running"?"⏸ Pause":"▶ Resume"}
                </button>
                <button onClick={logCountdown} style={{flex:1,padding:15,borderRadius:14,border:"none",cursor:"pointer",fontFamily:"var(--font-sans)",fontSize:15.5,fontWeight:700,color:"#fff",background:C.accentGrad,boxShadow:`0 8px 22px -8px ${C.accentGlow}`}}>
                  ✓ Log {timerLogMin}m
                </button>
              </div>
            )}
          ...
          <>
            ...
            <div style={{fontSize:20,fontWeight:600,margin:"12px 0 18px",lineHeight:1.3,color:C.text}}>
              {todayHasSession?"Nice — today's in the books.":prompt}
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setSheet({form:newForm(),editing:false,id:null})} style={{flex:1,padding:15,borderRadius:14,border:"none",cursor:"pointer",fontFamily:"var(--font-sans)",fontSize:15.5,fontWeight:700,color:"#fff",background:C.accentGrad,boxShadow:`0 8px 22px -8px ${C.accentGlow}`}}>
                {todayHasSession?"＋ Log another session":"＋ Log a session"}
              </button>
              <button onClick={openTimerSetup} style={{padding:"15px 18px",borderRadius:14,cursor:"pointer",border:`1px solid ${C.lineS}`,background:"transparent",color:C.muted,fontFamily:"var(--font-sans)",fontSize:15.5,fontWeight:600,display:"flex",alignItems:"center",gap:7}}>
                ▶ Timer
              </button>
            </div>
          </>
        ...
        <div style={{display:"flex",gap:7}}>
          {weekStrip.map((d,i)=>(
            <button key={d.ds} onClick={()=>setSheet({form:newForm(d.ds),editing:false,id:null})}
              ...
              <div style={{aspectRatio:"1",borderRadius:12,display:"grid",placeItems:"center",fontSize:14,fontWeight:600,
                background:d.logged?C.accentGrad:d.isToday?C.accentAlpha:C.surf2,
      ...
      <div className="card" style={card}>
            ...
            <div key={col} style={{display:"flex",flexDirection:"column",gap:4,flex:1}}>
              {Array.from({length:7}).map((_,row)=>{
                ...
                return <div key={row} title={cell&&!cell.future?`${cell.ds}: ${cell.mins}m`:""}
                  style={{aspectRatio:"1",borderRadius:3.5,background:cell?.future?"transparent":heatColor(cell?.mins||0,C.heat)}}/>;
              })}
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:5,alignItems:"center",justifyContent:"flex-end",marginTop:12,fontSize:11,color:C.dim}}>
          less
          {C.heat.map(c=><div key={c} style={{width:11,height:11,borderRadius:3,background:c}}/>)}
          more
        </div>
      </div>
          ...
          {calCells.map((ds,i)=>{
            ...
            return (
              <button key={ds} onClick={()=>setSheet({form:newForm(ds),editing:false,id:null})} style={{
                ...
                background:isToday?C.accentAlpha:hasSess?C.accentAlpha2:"transparent",
                ...
                {hasSess?<span style={{width:5,height:5,borderRadius:"50%",background:C.green}}/>
                  :missed?<span style={{width:5,height:5,borderRadius:"50%",background:C.surf2}}/>:<span style={{height:5}}/>}
              </button>
            );
          })}
        </div>
      ...
      <div className="card" style={card}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:projects.length?16:0}}>
          ...
          <button onClick={addProject} disabled={!newProject.trim()} style={{
            ...
            background:C.accentGrad,opacity:newProject.trim()?1:0.4,whiteSpace:"nowrap",
          }}>Add</button>
        </div>
        {projects.length>0&&(
            ...
            {projects.map(p=>(
              <div key={p.name} style={{background:C.surf2,borderRadius:14,padding:"13px 15px"}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:14.5,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",color:C.text}}>{p.name}</div>
      ...
      <div className="card" style={card}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:16}}>
          ...
          {recent.length>0&&(
            <button onClick={()=>setAllOpen(true)} style={{fontSize:12.5,fontWeight:600,color:C.indigo,background:C.accentAlpha2,border:"none",borderRadius:8,padding:"5px 12px",cursor:"pointer"}}>
              ...
              return (
                <div key={s.id} style={{background:C.surf2,border:`1px solid ${C.line}`,borderRadius:14,padding:"13px 14px",display:"flex",alignItems:"center",gap:10}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      {s.project&&(
                        <button onClick={()=>setNotesModal(s.project)} style={{fontSize:12.5,fontWeight:600,color:C.indigo,background:C.accentAlpha2,border:"none",borderRadius:7,padding:"3px 9px",cursor:"pointer",maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                          {s.project}{proj?.notes?" 📝":""}
                        </button>
                      )}
                      {s.note&&<span style={{fontSize:12.5,color:C.muted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.note}</span>}
                      {!s.project&&!s.note&&<span style={{fontSize:12.5,color:C.dim}}>session</span>}
